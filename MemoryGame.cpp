    ...
    //two extra blocks without actual data.
{
  ...
  for(int i = 0; i<numSlots; i++) {
    if(i < 6) values[arr[i]] = to_string(num[i/2]);
  else
    values[arr[i]] = "";
    }
  }
...
{   
  ...
  while(true){
      ...
      int num1,num2;
  cout << "Pick a cell to flip:    0     1     2     3     4     5     6     7"<<endl;
  cin >> num1;
    steps++;
  while(num1> 7 || num1 < 0){
    cout<<"index needs be in range of [0, 7]."<<endl;
    cout<<"Re-enter an index:    0     1     2     3     4     5     6     7"<<endl;
    cin >> num1;
    steps++;
  }
  if(bShown[num1] == true){
    cout<<"The cell indexed at "<<num1<<" is shown."<<endl;
  }
  if(bShown[num1] != true){
  ...
  cin >> num2;
   ...
   while(num2> 7 || num2 < 0){
    ...
    steps++;
     if(bShown[num2] == true){
    cout<<"The cell indexed at "<<num2<<" is shown."<<endl;
  }
  }
    
  if(values[num1]!="" && values[num1] == values[num2]) {
    bShown[num2] = true;
    answer++;
    } else bShown[num1] = false;
  display(bShown);
  } else {
  ...
  display(bShown);
    ...
    if(answer == 3){
      ...
      cout<<"Congratulations! Take " << steps <<"steps to find all matched pairs."<<endl;
      break;
    }
  }
